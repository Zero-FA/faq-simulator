...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!clientId || !channelId || !code) {
      return res.status(400).json({ error:'Missing clientId, channelId, or code' });
    }
    ...
    // If you need single-use enforcement across refreshes, plug a store here (Vercel KV/DB).
    // Without storage, we can't "consume" the code on the server.
    // Client stores success in localStorage, which survives refresh.
